...
<div class="flex h-full flex-col" wire:poll.5s="checkForNewPosts">
    @if ($newPostsCount > 0)
        <button
            class="sticky top-0 z-10 mx-auto mb-2 mt-2 flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-lg transition hover:bg-blue-500"
            wire:click="scrollToNewPosts">
            <flux:icon class="h-4 w-4" name="arrow-up" />
            {{ $newPostsCount }} new {{ Str::plural('post', $newPostsCount) }}
        </button>
    @endif
    <div class="flex-1 overflow-y-auto" id="feed-container">
        <div class="space-y-0">
            <div class="flex flex-col items-center justify-center py-20 text-center" wire:loading
            ...
            <div wire:loading.remove wire:target="refreshFeed">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if ($post->image_url)
                                    <div
                                        ...
                                        <img class="h-auto w-full object-cover" src="{{ $post->image_url }}"
                                            alt="Post image" onerror="this.parentElement.style.display='none'" />
                                    </div>
                                @endif
                                ...
                                <div class="mt-3 flex items-center gap-6 text-zinc-500 dark:text-zinc-400">
                                    <button class="flex items-center gap-1.5 text-xs transition hover:text-blue-500">
                                        <flux:icon class="h-4 w-4" name="chat-bubble-left-right" />
                                        <span>Reply</span>
                                    </button>
                                    <button class="flex items-center gap-1.5 text-xs transition hover:text-green-500">
                                        <flux:icon class="h-4 w-4" name="arrow-path-rounded-square" />
                                        <span>Repost</span>
                                    </button>
                                    @php
                                    ...
                                    @endphp
                                    <button
                                        class="{{ $isLiked ? 'text-red-500 font-semibold' : 'text-zinc-500 hover:text-red-500 dark:text-zinc-400' }} group flex items-center gap-1.5 text-xs font-medium transition duration-200 focus:outline-none"
                                        wire:click="toggleLike({{ $post->id }})" wire:loading.attr="disabled">

                                        {{-- Dynamic Heart Icon transformation --}}
                                        <flux:icon
                                            class="{{ $isLiked ? 'text-red-500' : 'text-zinc-400 dark:text-zinc-500 group-hover:text-red-500' }} h-4 w-4 transform transition-transform duration-200 group-active:scale-125"
                                            name="heart" :variant="$isLiked ? 'solid' : 'outline'" />
                ...
                @if ($loading)
                    <div class="flex items-center justify-center py-8" wire:loading.class="hidden">
                        <div class="h-6 w-6 animate-spin rounded-full border-2 border-zinc-300 border-t-blue-500"></div>
                    </div>
                @endif

                @if (!$hasMore && $posts->count() > 0)
                    <div class="py-6 text-center text-sm text-zinc-400 dark:text-zinc-500" wire:loading.class="hidden"
                        wire:target="refreshFeed">
                ...
                @if ($hasMore)
                    <div class="flex h-24 w-full shrink-0 items-center justify-center text-center" id="sentinel"
                        wire:loading.class="hidden" wire:target="refreshFeed">
